...
func (fn QueueExec) Run(ctx context.Context, flowName string, data map[string]interface{}) (string, error) {
	select {
	case <-ctx.Done():
		return "", ctx.Err()
	default:
...
func (fn QueueExec) Result(ctx context.Context, key, result string, timeOpts workflowQ.TimeOptions) (string, error) {
